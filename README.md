...
Сервис капчи с интерактивным слайдер-пазлом, построенный на Go и gRPC с использованием websocket.
Добавлен механизм блокировки пользователя через MAX_ATTEMTS количество попыток. 
Также реализована генерация капчи при каждом неудачном решении.
Добавлена complexity для изменения сложности задачи.
